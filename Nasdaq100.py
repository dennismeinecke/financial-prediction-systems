...
class AdvancedCrudeOilTradingSystem:  # Keep this name for compatibility
    def __init__(self):
        self.models = {}
        self.scalers = {}
    ...
    def get_current_nasdaq_price(self):
        """Get current Nasdaq E-mini price"""
        try:
            # Try CME Nasdaq E-mini Futures first
            futures_data = yf.download("NQ=F", period="5d", interval="1h")
            if not futures_data.empty and len(futures_data) > 0:
                current_price = float(futures_data['Close'].iloc[-1])
                if not np.isnan(current_price) and 10000 < current_price < 30000:
                    print("Using CME Nasdaq E-mini Futures (NQ=F): $" + "{:.2f}".format(current_price))
                    return current_price, "NQ=F"
            
            # Try Nasdaq 100 Index directly
            index_data = yf.download("^NDX", period="5d", interval="1h")
            if not index_data.empty and len(index_data) > 0:
                index_price = float(index_data['Close'].iloc[-1])
                # Convert index to futures approximation
                current_price = index_price
                if not np.isnan(current_price) and 10000 < current_price < 30000:
                    print("Using Nasdaq 100 Index (^NDX): $" + "{:.2f}".format(current_price))
                    return current_price, "^NDX"
                
        except Exception as e:
            print("Error getting current Nasdaq price: " + str(e))
        
        # Fallback
        print("Using fallback Nasdaq price: $18000.00")
        return 18000.0, "FALLBACK"
    
    def get_historical_data(self, symbol="NQ=F"):
        """Get historical Nasdaq E-mini data"""
        try:
    ...
    def create_safe_features(self, df):
        """Create safe technical features without column errors"""
        df = df.copy()
        ...
        # CREATE BASIC RETURN COLUMN FIRST
        ...
        for window in windows:
            if 'Close' in df.columns:
                ...
                df['SMA_' + str(window)] = 0
                df['EMA_' + str(window)] = 0
                df['WMA_' + str(window)] = 0
        ...
        if 'SMA_13' in df.columns and 'SMA_55' in df.columns:
            df['SMA_13_55_ratio'] = np.where(df['SMA_55'] != 0, df['SMA_13'] / df['SMA_55'], 1)
        else:
            df['SMA_13_55_ratio'] = 1
            
        if 'EMA_5' in df.columns and 'SMA_21' in df.columns:
        ...
        # Fast MACD for Nasdaq volatility
        if 'Close' in df.columns:
        ...
        if all(col in df.columns for col in ['Low', 'High', 'Close']):
        ...
        df['Wedding_Season'] = ((df['Month'] == 10) | (df['Month'] == 11) | (df['Month'] == 12)).astype(int)  # Wedding season
        df['Investment_Season'] = ((df['Month'] == 1) | (df['Month'] == 12)).astype(int)  # Investment season
        df['Golden_Week'] = ((df['Month'] == 5) & (df['DayOfMonth'] >= 1) & (df['DayOfMonth'] <= 5)).astype(int)  # Golden Week
        df['Obon_Season'] = ((df['Month'] == 8) & (df['DayOfMonth'] >= 13) & (df['DayOfMonth'] <= 15)).astype(int)  # Obon festival
    ...
    def prepare_data(self, df):
        ...
        feature_columns = [
            ...
            'Wedding_Season', 'Investment_Season', 'Golden_Week', 'Obon_Season',
    ...
    def analyze_cultural_impact(self, df):
        """Analyze cultural calendar impact on Nasdaq prices"""
        try:
            # Get recent data
            if len(df) < 50:
                return {
    ...
    def run_analysis(self):
        """Run complete advanced Nasdaq trading analysis"""
        print("="*70)
        print("ADVANCED CME NASDAQ E-MINI TRADING SYSTEM")
        print("WITH CULTURAL CALENDAR INTEGRATION")
        print("="*70)
        
        # Get current Nasdaq price
        current_price, source = self.get_current_nasdaq_price()
        print("Data Source: " + source)
        print()
        
        # Get historical data
        symbol = "NQ=F" if source == "NQ=F" else ("^NDX" if source == "^NDX" else "NQ=F")
        ...
        print("\n" + "="*80)
        print("CME NASDAQ E-MINI MODEL RESULTS")
        print("="*80)
        ...
        print("\n💰 CME Nasdaq E-mini Futures Details:")
        print("-"*40)
        print("  Contract Size: 1000 troy ounces")
        print("  Tick Size: 0.1 troy ounce ($10)")
...
# Risk Management System for Gold Futures
...
try:
    # Run the advanced Nasdaq trading system
    print("Initializing Advanced CME Nasdaq E-mini Trading System...")
    nasdaq_system = AdvancedCrudeOilTradingSystem()  # CORRECT CLASS NAME
    results = nasdaq_system.run_analysis()
    
    if results:
        print("\n" + "="*50)
        print("NASDAQ E-MINI TRADING ANALYSIS COMPLETED")
        print("="*50)
        ...
        if cultural_impact['holiday_avg_return'] > 0:
            print("  • Nasdaq tends to rise during holidays")
            print("  • Consider long positions before major holidays")
        else:
            print("  • Nasdaq tends to consolidate during holidays")
        ...
        print("❌ Nasdaq trading analysis failed")
        
except Exception as e:
    print("❌ Error running Nasdaq trading system: " + str(e))
    print("\nTrying simplified fallback...")
    
    # Simplified fallback
    try:
        print("Getting current Nasdaq price...")
        data = yf.download("NQ=F", period="1d", interval="1h")
        if not data.empty:
            current_price = float(data['Close'].iloc[-1])
        else:
            data = yf.download("^NDX", period="1d", interval="1h")
            if not data.empty:
                current_price = float(data['Close'].iloc[-1])
            else:
                current_price = 18000.0
        
        print("Current Nasdaq E-mini Price: $" + "{:.2f}".format(current_price))
...
GoldRiskManager.calculate_margin_requirement = calculate_margin_requirement